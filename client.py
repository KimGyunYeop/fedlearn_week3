...
def main(DEVICE, gpu, port):
    ...
    # Start client
    fl.client.start_numpy_client(server_address="127.0.0.1:"+str(port), client=CifarClient())
...
if __name__ == "__main__":
    ...
    ps = []
    for i in list:
        p =Process(target=main, args=(i, args.gpu, args.port, ))
        ps.append(p)
        p.start()
    for p in ps:
        p.join()
